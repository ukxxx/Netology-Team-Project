...
def set_params_to_match(user):
    ...
    if (
        user["sex"] == 1
    ):  # If the user's gender is female, change to male for opposite-sex matches
        user["sex"] = 2
    else:
        user["sex"] = 1
...
for event in longpoll.listen():
    ...
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        ...
        if event.text == "💓 Начать 💓":
            ...
            write_msg(
                event.user_id, f"Может быть это твоя любовь?", show_keyboard_first()
            )
            ...
            if not ids:
                ...
                write_msg(
                    event.user_id,
                    f"{phrases[randrange(len(phrases))]}",
                    show_keyboard_main(),
                )
            ...
            if not ids:
                ...
                write_msg(
                    event.user_id,
                    f"Давай сначала подберем тебе пару",
                    show_keyboard_first(),
                )
                ids += go_first(event.user_id)
            else:
                ...
                write_msg(
                    event.user_id, f"Сохранено, едем дальше", show_keyboard_main()
                )
            ...
            write_msg(
                event.user_id, f"Не понял тебя, начнем поиск?", show_keyboard_first()
            )
